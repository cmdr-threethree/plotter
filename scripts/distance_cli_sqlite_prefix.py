...
def find_path_directional(
    ...
    def _step(
        ...
        while radius <= current_max_hop:
            ...
            cands = neighbors_for_center_prefix(
                conn,
                fake_center,
                radius,
                set(),
...
def _find_path_robust_single(
    ...
    if dist > 0:
        ...
        for i in range(waypoint_tries):
            ...
            mid_sys = nearest_system(
                conn, {"x": wx, "y": wy, "z": wz}, coord_scale, id_to_prefix, id_to_star
            )
            if (
                not mid_sys
...
def choose_candidate_list(lst: List[Dict], which: str) -> Optional[Dict]:
    ...
    if len(lst) == 1:
        s = lst[0]
        print(
            f"Selected: {s['name']} (id64={s['id64']}) coords={s['coords']} mainStar={s.get('mainStar')}"
        )
        return s
    ...
    for i, s in enumerate(lst, start=1):
        c = s["coords"]
        print(
            f" {i}) {s['name']} id64={s['id64']} coords=({c['x']},{c['y']},{c['z']}) mainStar={s.get('mainStar')}"
        )
    while True:
...
def main():
    parser = argparse.ArgumentParser(
        description="SQLite-backed distance/path tool with optional prefix compression"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Scale factor for storing coordinates as integers (default 32)",
    )
    ...
    parser.add_argument(
        "--max-hop", type=float, help="If provided, find path where each hop <= this"
    )
    parser.add_argument("--max-nodes", type=int, default=5000)
    parser.add_argument("--max-neighbors", type=int, default=500)
    parser.add_argument(
        "--only-star-types",
        help="Comma-separated star type names to restrict nodes used in pathfinding",
    )
    parser.add_argument(
        "--nearest-type",
        help="Find nearest system of given star type name (comma-separated supported)",
    )
    parser.add_argument(
        "--near",
        help='Reference point for --nearest-type: either "id64"/name or coordinates "x,y,z"',
    )
    parser.add_argument(
        "--directional",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Initial threshold distance when searching near the step point",
    )
    parser.add_argument(
        "--step-expand-factor",
        type=float,
        default=2.0,
        help="Multiplier to expand search radius when no candidate found",
    )
    parser.add_argument(
        "--waypoint-tries",
        type=int,
        default=50,
        help="Number of random waypoints to try if direct path fails",
    )
    parser.add_argument(
        "--preload",
        action="store_true",
        help="Preload buckets between source and target into memory to avoid SQL roundtrips",
    )
    parser.add_argument(
        "--preload-margin-buckets",
        type=int,
        default=1,
        help="Extra bucket margin when preloading",
    )
    ...
    if args.nearest_type is not None or args.near is not None:
        ...
        if "," in near:
            ...
            cand = get_system_by_query_prefix(
    ...
    cand1 = get_system_by_query_prefix(
        conn, q1, meta, id_to_prefix, id_to_star, coord_scale
    )
    ...
    cand2 = get_system_by_query_prefix(
        conn, q2, meta, id_to_prefix, id_to_star, coord_scale
    )
    ...
    if args.preload:
        ...
        def bucket_coords_of(c):
            return (
                math.floor(c["x"] / bucket_size),
                math.floor(c["y"] / bucket_size),
                math.floor(c["z"] / bucket_size),
            )
        ...
        print(
            f"Preloading buckets bx[{min_bx}..{max_bx}] by[{min_by}..{max_by}] bz[{min_bz}..{max_bz}]"
        )
        in_memory_buckets = {}
        ...
        cur = conn.execute(
            sql, (s_bs, min_bx, max_bx, s_bs, min_by, max_by, s_bs, min_bz, max_bz)
        )
        for r in cur:
