...
def api_search():
    ...
    try:
        ...
        results = distance.get_system_by_query_prefix(
            conn, q, META, ID_TO_PREFIX, ID_TO_STAR, coord_scale, limit=20
        )
    finally:
        conn.close()
...
def api_nearest():
    ...
    try:
        ...
        if not near_coords:
            cands = distance.get_system_by_query_prefix(
                conn, near_q, META, ID_TO_PREFIX, ID_TO_STAR, coord_scale, limit=1
            )
            if cands:
...
def api_path():
    ...
    try:
        ...
        s_list = distance.get_system_by_query_prefix(
            conn, source_q, META, ID_TO_PREFIX, ID_TO_STAR, coord_scale, limit=10
        )
        t_list = distance.get_system_by_query_prefix(
            conn, target_q, META, ID_TO_PREFIX, ID_TO_STAR, coord_scale, limit=10
        )
        if not s_list:
